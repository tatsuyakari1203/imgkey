...
- Current source of truth is the root source files, `ImgKey.spec`, `ImgKey-GPU.spec`, `docs/build-gpu.md`, and the completed v11 large-image acceleration plan. `build/`, `dist/`, native build outputs, caches, and `.artifact/` outputs are generated/disposable.

## Core architecture
- `app.py` — PySide6 viewer-first UI, `ImageCanvas`, inspector controls, cache-aware preview/export orchestration, eyedropper, manual keep/remove masks, imported matte support, GPU status probe, Fast PNG option, and full-resolution PNG export wiring.
- `keyer.py` — image I/O helpers, preview resize, compatible `KeySettings`/`KeyResult`, optional process-cache integration, global matte/trimap logic, linear-light fringe/edge color repair, guided alpha refinement, tile-local fallbacks, crop render support, tile export, despill, and checkerboard compositing.
- `imgkey_engine/cache.py` / `cache_keys.py` / `profiling.py` — active-image process cache records, conservative generation/fingerprint helpers, and reusable stage timing hooks used by preview/export and smoke-test benchmark reports.
...
- `smoke_test.py` — synthetic smoke tests for green/blue/custom keying, connected-background preservation, edge alpha, linear-light repair, guided alpha, tile-local screen/nearest-inner fallbacks, crop/cache parity, PNG option parity, large-image profiling, despill, tile consistency, removed runtime-surface guards, and import fences.
...
- `python smoke_test.py --profile-large-images C:\Users\Admin\Downloads\zzz` — optional real-image profiler; writes JSON/markdown under `.artifact/large-image-perf/` and falls back to synthetic fixtures if the path is absent.
- `python smoke_test.py --write-perf-baseline` — optional synthetic large-image perf baseline under `.artifact/perf/`.
- PowerShell compile check: `$files = @("app.py", "keyer.py", "smoke_test.py", "gpu_runtime.py", "screen_analysis.py", "gpu_accel.py", "gpu_backend.py", "native_toolchain.py", "vulkan_runtime.py", "subprocess_utils.py", "packaging/pyinstaller/rthooks/imgkey_cuda_runtime.py") + (Get-ChildItem -Path "imgkey_engine", "ui" -Filter "*.py").FullName; python -m py_compile @files`.
...
- Process-cache rule: source/proxy/base matte/transition/tile-prep/color records are separate. Full export must never reuse proxy matte data; exact Full Crop can seed full export only through full-resolution cache keys. Source/mask/imported-matte generation counters and conservative settings fingerprints must invalidate cache before reuse; stale/cancelled preview jobs must discard staged cache records.
- Preview responsiveness rule: Proxy is fast whole-image preview; Full Crop is an exact pinned full-resolution ROI that may still wait for full-image matte work. Pan/zoom must not silently repin exact crop, slider drags should use latest-wins draft/proxy scheduling, and no multiple concurrent 25MP preview jobs should be queued.
...
- Fast PNG rule: default export remains lossless PNG compression level 6; **Fast PNG** uses level 1 for identical decoded pixels and larger files. Do not change default compression or output pixels without explicit approval.
...
- Run `python smoke_test.py`, `--write-geometric-benchmark`, `--tune-geometric-defaults`, GPU parity/benchmark, `--write-perf-baseline`, `python -m gpu_runtime --probe --json`, the required `py_compile`, `python -c "import app, keyer; print('import ok')"`, the dependency fence, the primary PyInstaller build, packaged EXE probes from temp cwd/sanitized PATH, GUI lifetime smoke, and archive checks before marking a release-hardening phase complete. Use `docs/build-gpu.md` for native backend packaging commands and clean-target expectations.
